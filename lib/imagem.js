...
const MINIMO = { hero: 1800, destaque: 1200, promo: 900, produto: 800 };
...
function decodificar(arquivo) {
  return new Promise((ok, falha) => {
    ...
    const relogio = setTimeout(() => {
      ...
      falha(new Error(
        `O navegador travou ao abrir "${arquivo.name}" (${(arquivo.size / 1048576).toFixed(1)} MB). ` +
        'Isso costuma acontecer com fotos muito grandes de câmera. Reduza a imagem antes de enviar, ' +
        'ou abra e salve de novo em algum editor para gerar um JPEG mais leve.'
      ));
    }, 15000);
